 ...
 */
import { newRequestId } from "./events.js";
/**
 ...
 */
export async function wrapEvents(events, meta, fn, extractEnd) {
 ...
 */
export async function* wrapStream(events, meta, source, extractDelta) {
    const id = newRequestId();
    const startedAt = Date.now();
    events.emit("request:start", {
        id,
    ...
    try {
        for await (const chunk of source()) {
            ...
            if (info.completionTokens !== undefined)
                lastUsageTokens = info.completionTokens;
            if (info.costUSD !== undefined)
                costUSD = info.costUSD;
            if (info.finishReason !== undefined)
...
//# sourceMappingURL=event-wrap.js.map