 ...
 * ```
 */
export function encodeImageUrl(bytes) {
    // Normalize to Uint8Array so the rest of the function is uniform.
    if (bytes instanceof Blob) {
        throw new Error("encodeImageUrl: Blob input requires async; await blob.arrayBuffer() and pass the result instead");
    }
 ...
 * ```
 */
export async function decodeMediaUrl(url) {
